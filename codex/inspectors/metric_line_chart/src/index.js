(function () {
  ...
  function AthenaBridge() {
    this._pending = {};   // request_id -> {resolve, reject}
    this._subs = {};      // subscription_id -> callback
    this._initData = null;
  ...
  AthenaBridge.prototype._onMessage = function (event) {
    ...
    switch (msg.type) {
      case 'init':
        this._initData = msg;
        if (this._initResolve) { this._initResolve(msg); this._initResolve = null; }
        break;
      ...
      case 'artifact_data':
        if (msg.request_id && this._pending[msg.request_id]) {
          if (msg.error) { this._pending[msg.request_id].reject(new Error(msg.error)); }
          else { this._pending[msg.request_id].resolve(msg); }
          delete this._pending[msg.request_id];
        }
        break;
  ...
  AthenaBridge.prototype._request = function (msg) {
    ...
    return new Promise(function (resolve, reject) {
      self._pending[msg.request_id] = { resolve: resolve, reject: reject };
  ...
  AthenaBridge.prototype.queryMetrics = function (runIds, metricNames) {
    ...
    return this._request({
      type: 'query_metrics', request_id: id,
      run_ids: runIds, metric_names: metricNames,
    });
  };
  ...
  AthenaBridge.prototype.listMetrics = function (runId) {
    ...
    return this._request({
      type: 'list_metrics', request_id: id, run_id: runId,
    });
  };
  ...
  AthenaBridge.prototype.queryEvents = function (runId, filter, limit) {
    ...
    return this._request({
      type: 'query_events', request_id: id,
      run_id: runId, filter: filter, limit: limit,
    });
  };
  ...
  AthenaBridge.prototype.fetchArtifact = function (artifactId, asJson) {
    ...
    return this._request({
      type: 'fetch_artifact', request_id: id,
      artifact_id: artifactId, as_json: !!asJson,
    });
  };
  ...
  AthenaBridge.prototype.searchArtifacts = function (filter) {
    ...
    return this._request({
      type: 'search_artifacts', request_id: id, filter: filter,
    });
  };
  ...
  AthenaBridge.prototype.subscribeEvents = function (runId, filter, callback) {
    ...
    window.parent.postMessage({
      type: 'subscribe_events', subscription_id: subId,
      run_id: runId, filter: filter,
    }, '*');
    return subId;
  };
  ...
  AthenaBridge.prototype.saveState = function (state) {
    window.parent.postMessage({ type: 'save_state', state: state }, '*');
  };
  ...
  function MetricPicker(bridge, options) {
    this.bridge = bridge;
  ...
  // State
  var selectedMetric = '';
  var runData = {};           // runId -> {values:[], labels:[], color:'', name:''}
  var smoothingAlpha = 0;     // 0 = off, > 0 = EMA alpha
  var logScale = false;
  var runIds = [];
  var runContexts = [];
  var subscriptionIds = [];
  ...
  scaleBtn.addEventListener('click', function () {
    ...
    if (logScale) scaleBtn.classList.add('active');
  ...
  function loadMetricData() {
    ...
    bridge.queryMetrics(runIds, [selectedMetric]).then(function (resp) {
      ...
      // metricsMap is run_id -> metric_name -> MetricPoint[]
      ...
      status.textContent = selectedMetric + ' \u00B7 ' + totalPoints() + ' points';
      render();
    }).catch(function (err) {
  ...
  function totalPoints() {
    ...
    for (var k in runData) {
      if (runData.hasOwnProperty(k)) n += runData[k].values.length;
    }
    return n;
  }
  ...
  function render() {
    ...
    var allValues = [];
    var allLabels = [];
    var series = []; // {runId, points:[{x,y}], color, name}

    for (var i = 0; i < runIds.length; i++) {
      var rid = runIds[i];
      var rd = runData[rid];
      if (!rd || !rd.values.length) continue;
      ...
      for (var j = 0; j < vals.length; j++) {
        var yVal = vals[j];
        if (logScale && yVal > 0) yVal = Math.log10(yVal);
        else if (logScale && yVal <= 0) yVal = -10; // floor for log
        pts.push({ x: j, y: yVal });
        allValues.push(yVal);
        allLabels.push(rd.labels[j]);
      }
      series.push({
        runId: rid, points: pts,
    ...
    for (var xi = 0; xi < xTickCount; xi++) {
      ...
      // Try to use actual label (step), fall back to index
      var labelVal = idx;
      // Find the first series that has this index
  ...
  canvas.addEventListener('mousemove', function (e) {
    ...
    if (mouseX < pad.left || mouseX > rect.width - pad.right ||
        mouseY < pad.top || mouseY > rect.height - pad.bottom) {
      tooltip.style.display = 'none';
      return;
    }
    ...
    for (var hi = 0; hi < hits.length; hi++) {
      ...
      if (logScale) rawVal = Math.pow(10, rawVal);
  ...
  bridge.waitInit().then(function (init) {
    ...
    // Fallback: single run_id from init (top-level or inside inputs)
    ...
    // Load metric picker from first run
    picker.load(runIds[0]).then(function () {
      // If saved_state has a metric, restore it
    ...
    // Subscribe to live events for each run
    for (var si = 0; si < runIds.length; si++) {
      (function (rid) {
        var subId = bridge.subscribeEvents(rid, { event_type: 'metric' }, function (event) {
          // CRITICAL: event.event_name (NOT event.name), event.payload.value (NOT event.value)
          var name = event.event_name;
          var payload = event.payload || {};
          if (!name || payload.value === undefined) return;
          if (name !== selectedMetric) return;
