(function () {
  ...
  // ==========================================================================
  // THEME — shared Athena dark palette
  // ==========================================================================
  var THEME = {
    ...
    headerPad: '12px 16px 4px', statusPad: '2px 16px 8px',
  };

  // ==========================================================================
  // Base + gallery-specific styles
  // ==========================================================================
  ...
  // ==========================================================================
  // AthenaBridge — postMessage wrapper for the inspector iframe protocol
  // ==========================================================================
  function AthenaBridge() {
    ...
    self._readyPromise = new Promise(function (resolve) { self._readyResolve = resolve; });
    ...
    window.addEventListener('message', function (event) {
      ...
      if (msg.type === 'init') {
        ...
        // Build run meta from init data
        var runs = msg.runs || [];
        for (var i = 0; i < runs.length; i++) {
          self._runMeta[runs[i].id] = runs[i];
        }
        // If runs not provided, create basic entries from run_ids
        if (runs.length === 0) {
        ...
        if (self._readyResolve) { self._readyResolve(msg); self._readyResolve = null; }
        self._emit('init', msg);
      } else if (msg.type === 'artifact_data') {
        var cb = self._pending[msg.request_id];
        if (cb) { delete self._pending[msg.request_id]; cb(null, msg); }
      } else if (msg.type === 'artifact_meta') {
        var cb2 = self._pending[msg.request_id];
        if (cb2) { delete self._pending[msg.request_id]; cb2(null, msg); }
      } else if (msg.type === 'data') {
        var cb3 = self._pending[msg.request_id];
        if (cb3) { delete self._pending[msg.request_id]; cb3(null, msg); }
      } else if (msg.type === 'event') {
        self._emit('event:' + msg.subscription_id, msg.event);
      } else if (msg.type === 'error') {
        var cb4 = self._pending[msg.request_id];
        if (cb4) { delete self._pending[msg.request_id]; cb4(msg, null); }
      }
    });
  }

  AthenaBridge.prototype._nextId = function () { return 'req_' + (++this._reqId); };
  ...
  AthenaBridge.prototype.ready = function () { return this._readyPromise; };

  AthenaBridge.prototype.getRunIds = function () { return this._runIds; };
  ...
  AthenaBridge.prototype.fetchArtifact = function (artifactId, asJson) {
    ...
    return new Promise(function (resolve, reject) {
      self._pending[id] = function (err, data) {
        if (err) return reject(new Error(err.message || 'fetch failed'));
        resolve(data);
      };
  ...
  AthenaBridge.prototype.getArtifactMeta = function (artifactId) {
    ...
    return new Promise(function (resolve, reject) {
      self._pending[id] = function (err, data) {
        if (err) return reject(new Error(err.message || 'meta fetch failed'));
        resolve(data);
      };
      window.parent.postMessage({
  ...
  AthenaBridge.prototype.searchArtifacts = function (filter) {
    ...
    return new Promise(function (resolve, reject) {
      self._pending[id] = function (err, data) {
        if (err) return reject(new Error(err.message || 'search failed'));
        resolve(data);
      };
      window.parent.postMessage({
  ...
  // ==========================================================================
  // Gallery state (mutable — no stale closures)
  // ==========================================================================
  var images = [];           // { artifactId, name, node_id, tags, step, epoch, timestamp, blobUrl, loaded }
  var pendingArtifacts = {}; // dedup set
  ...
  function tryAddArtifact(artifactId, runId) {
    if (!artifactId || pendingArtifacts[artifactId]) return;
    pendingArtifacts[artifactId] = true;

    bridge.getArtifactMeta(artifactId).then(function (meta) {
      // Check if this is likely an image artifact.
      // content_type is often not stored on the server, so also check
      // name/storage_ref extension and schema_type as fallbacks.
      ...
      if (!isImage) {
        for (var ei = 0; ei < imageExts.length; ei++) {
          if (name.indexOf(imageExts[ei]) !== -1) { isImage = true; break; }
        }
      }
  ...
  function updateImageSrc(artifactId, url) {
    ...
    if (img) {
      ...
      // Card might have placeholder text; re-render to swap it
      renderGallery();
    }
  }
  ...
  function renderGallery() {
    ...
    for (var i = 0; i < images.length; i++) {
      ...
      // Meta row: node badge + step/epoch
      ...
      if (entry.epoch != null) parts.push('epoch ' + entry.epoch);
      if (parts.length > 0) {
      ...
      // Click to open lightbox (use closure to capture blobUrl)
      (function (e) {
        card.addEventListener('click', function () {
          if (e.blobUrl) openLightbox(e.blobUrl);
        });
      })(entry);
  ...
  // ==========================================================================
  // Init: wait for bridge ready, then subscribe + backfill
  // ==========================================================================
  injectBaseStyles();
