(() => {
  ...
  (function() {
    ...
    AthenaBridge.prototype._dispatch = function(msg) {
      ...
      if (type === "init") {
        this.runs = (msg.run_ids || [msg.run_id]).map(function(id, i2) {
          return {
            id,
            name: msg.run_names ? msg.run_names[i2] : null,
            number: msg.run_numbers ? msg.run_numbers[i2] : null
          };
        });
    ...
    AthenaBridge.prototype._request = function(msg) {
      ...
      var p = new Promise(function(resolve, reject) {
        self._pending[id] = { resolve, reject };
      });
    ...
    AthenaBridge.prototype.getRunIds = function() {
      return this.runs.map(function(r) {
        return r.id;
      });
    };
    ...
    function recordValue(metricName, runId, value) {
      ...
      m.values.push(value);
      if (m.values.length > 100)
        m.values = m.values.slice(-100);
      m.current = value;
      if (value < m.min)
        m.min = value;
      if (value > m.max)
    ...
    function updateCard(metricName, runId, color) {
      ...
      card.stats.textContent = "min " + formatValue(primary.min) + " / max " + formatValue(primary.max) + " / mean " + formatValue(mean);
